...
const assetsHeader = [
  ...
  {
    name: "Side Chain Balance",
    sortable: sortingTypes.SORT_BY_SIDECHAIN_BALANCE,
  },
...
const sortAndFilter = (
    ...
    switch (sortBy) {
      case sortingTypes.SORT_BY_NAME:
        return keys.sort((a, b) =>
          tokenNames[a] > tokenNames[b] ? plus : minus
        );
      ...
      case sortingTypes.SORT_BY_SIDECHAIN_BALANCE:
        return keys.sort((a, b) =>
          data[a].sidechain.gt(data[b].sidechain) ? plus : minus
        );
  ...
  const hideEmpty = (data: BalancesType) => {
    ...
    return keys.filter(
      (key) =>
        data[key].resardis.gt(0) ||
        data[key].sidechain.gt(0) ||
        data[key].mainnet.gt(0)
    );
  };
...
const Assets = ({
  ...
  const toggleHidingEmptyBalances = () =>
    setHideEmptyBalances(!hideEmptyBalances);
    ...
    <div className="container">
      <div className="assets-top-header">
        ...
        <div className="assets-total">
          Total Balance: {totalBalanceUSD.toFixed()} USD /{" "}
          {totalBalanceBTC.toFixed()} BTC
        </div>
      </div>
        ...
        <table className="assets-table">
          <tbody>
            {sortAndFilter(
              ...
              const tokenMarketData = marketData[`${symbol}/ETH`]
                ...
                <tr key={symbol}>
                  ...
                  {hasMarketData ? (
                    <>
                      <td
                        style={{
                          color: tokenMarketData[1] < 0 ? "#D5002A" : "#00AA55",
                        }}
                      >
                        {tokenMarketData[1]}%
                      </td>
