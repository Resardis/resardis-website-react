...
headersAll = {
  ...
  [TAB_USERDATA_FUNDS]: [
    ...
    { display: "My Wallet", sortable: sortingTypes.SORT_BY_AVAILABLE },
    {
      display: "Deposited to Resardis",
      sortable: sortingTypes.SORT_BY_BALANCE_IN_USE,
    },
  ],
  [TAB_USERDATA_ORDER_HISTORY]: [
...
export const sortAndFilter = (
    ...
    switch (sortBy) {
      case sortingTypes.SORT_BY_DATE:
        return keys.sort((a, b) =>
          data[a].timestamp > data[b].timestamp ? plus : minus
        );
      case sortingTypes.SORT_BY_TYPE:
        return keys.sort((a, b) =>
          data[a].type > data[b].type ? plus : minus
        );
      case sortingTypes.SORT_BY_SIDE:
        return keys.sort((a, b) =>
          data[a].side > data[b].side ? plus : minus
        );
      case sortingTypes.SORT_BY_PRICE:
        return keys.sort((a, b) =>
          new BigNumber(data[a].price).gt(new BigNumber(data[b].price))
            ? plus
            : minus
        );
      case sortingTypes.SORT_BY_AMOUNT:
        return keys.sort((a, b) =>
          new BigNumber(data[a].amount).gt(new BigNumber(data[b].amount))
            ? plus
            : minus
        );
      case sortingTypes.SORT_BY_FILLED:
        return keys.sort((a, b) =>
          new BigNumber(data[a].filled).gt(new BigNumber(data[b].filled))
            ? plus
            : minus
        );
      case sortingTypes.SORT_BY_AVERAGE:
        return keys.sort((a, b) =>
          new BigNumber(data[a].average).gt(new BigNumber(data[b].average))
            ? plus
            : minus
        );
      case sortingTypes.SORT_BY_TOTAL:
        return keys.sort((a, b) =>
          new BigNumber(data[a].total).gt(new BigNumber(data[b].total))
            ? plus
            : minus
        );
  ...
  const runActiveFilter = (data: MyOrdersListType) => {
    ...
    if (showOrders === "active")
      return keys.filter(
        (id: string) => id in activeOffers && activeOffers[id]
      );
    if (showOrders === "closed")
      return keys.filter(
        (id: string) => id in activeOffers && !activeOffers[id]
      );
    ...
    return (
      <div className="no-wallet-info">
        ...
        <p>
          ...
          <a href="https://metamask.io/" style={{ color: "#FBB03B" }}>
            MetaMask
          </a>
          ?
        </p>
          ...
          onClick={() => {
            ...
            window.ethereum
              ...
              .catch((err: any) => {
                if (err.code === 4001) {
                  if (buttonElement)
                    buttonElement.innerHTML = "Request denied?";
                } else {
                  if (buttonElement)
...
const Table = ({ headers, activeTab, isWalletEnabled }: PropsFromRedux) => {
  ...
  const flipSortDirection = () => {
    setSortDirection(
      sortDirection === sortingDirections.SORT_DESC
        ? sortingDirections.SORT_ASC
        : sortingDirections.SORT_DESC
    );
  };

  if (!isWalletEnabled) return <NoWallet />;

  if (activeTab == TAB_USERDATA_DEPOSIT) {
    return <Deposit />;
  }

  if (activeTab == TAB_USERDATA_WITHDRAW) {
    return <Withdraw />;
  }
...
export default connector((props: PropsFromRedux) => <Table {...props} />);
