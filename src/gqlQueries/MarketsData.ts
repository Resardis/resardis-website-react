...
export const getPairsSubscription = (last24Start:number, prev24Start:number, prev24End:number) => {
  const GET_PAIRS_SUBSCRIPTION = `
  subscription getPairsSubscription {
    last24takes: takes(where: { timestamp_gt: "${last24Start}" } orderBy: timestamp orderDirection: desc) {
      payGem
      buyGem
      takeAmt
  ...
  return gql(GET_PAIRS_SUBSCRIPTION)
}
